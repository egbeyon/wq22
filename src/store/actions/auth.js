...
import { postAlert, setAlert } from './alert';
...
export const login = (username, password) => async dispatch => {
  try {
    ...
    if (response.ok) {
      ...
      dispatch(postAlert({title: 'userLogin', message: `${user.name} login, success'`}))
    }
    if (responseData.error) {
      dispatch({ type: LOGIN_FAIL });
      dispatch(setAlert(responseData.error.message, 'error', 5000));
      dispatch(postAlert({title: 'userLogin', message: `{responseData.error.message}` }))
    }
  } catch (error) {
    dispatch({ type: LOGIN_FAIL });
    dispatch(setAlert(error.message, 'error', 5000));
...
export const logout = () => async dispatch => {
  try {
    ...
    if (response.ok) {
      ...
      dispatch(postAlert({title: 'userLogout', message: 'LOGOUT Success, success' }))
